...
def test_patch_user(client, auth_headers, new_user):
    ...
    """The server responds with an error if a patch for a pw reset is made and the current pw does not match the pw in the db"""
    responseInvalidCurrentPassword = client.patch(f"api/user/{userToPatch.id}", json={"currentPassword": "incorrect", "newPassword": "newPassword"},
        headers=auth_headers["admin"])
    assert responseInvalidCurrentPassword.status_code == 400
